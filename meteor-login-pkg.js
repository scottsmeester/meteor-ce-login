...
if (Meteor.isClient) {
  ...
  Template.statusPieChart.rendered = function() {
    Tracker.autorun(function(){
      buildStageBreakdown();
    });
  }

  // var stageBreakdownPie;

  function buildStageBreakdown() {

    var chartData = [],
        stageBreakdownPie,
        stageBreakdownFunnel;

    console.log('Sessions', Session.get('myLists')[0].id)
    ...
    stageBreakdownPie = $('#stageBreakdownPie').highcharts({
        title: {
    ...
    stageBreakdownFunnel = $('#stageBreakdownFunnel').highcharts({
      chart: {
                  type: 'area'
              },
              title: {
